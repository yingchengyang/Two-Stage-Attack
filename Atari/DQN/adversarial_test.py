...
def run_attack(input, model, attack_method, epsilon, steps, target_model, norm=2):
    if epsilon == 0.0:
        return input
    # print(attack_method)
    if attack_method == 'none':
        ...
        # l_infty
            ...
            input = input + input.grad / torch.max(torch.abs(input.grad)) * epsilon * 80.0
        return input
        ...
        for _ in range(steps):
            ...
            if norm == 2:
                ...
                input = input - input.grad / torch.max(torch.abs(input.grad)) * epsilon_step * 80.0
...
def run_trajectory(model, env, attack_method, args, epsilon, target_model=None, norm=2):
    ...
    while True:
        ...
        input_x = run_attack(input_x, model, attack_method, epsilon, args.steps, target_model, norm)
...
def main(args):
    ...
    if args.target_attack:
        ...
        if not os.path.exists('data/' + task + '/' + args.attack_method + norm_name):
            os.mkdir('data/' + task + '/' + args.attack_method + norm_name)
        f = open('data/' + task + '/' + args.attack_method + norm_name + '/' + save_name + '_' + neg_name + '.txt', "w")
    else:
        if not os.path.exists('data/' + task + '/' + args.attack_method + norm_name):
            os.mkdir('data/' + task + '/' + args.attack_method + norm_name)
        f = open('data/' + task + '/' + args.attack_method + norm_name + '/' + save_name + '.txt', "w")
...
if __name__ == '__main__':
    ...
    # attack1 = ['untarget_kl_pgd']
    attack1 = []
    attack2 = ['target_kl_pgd']
    if args.env == 'PongNoFrameskip-v4':
        ta = "Pong"
    elif args.env == 'QbertNoFrameskip-v4':
        ta = "Qbert"

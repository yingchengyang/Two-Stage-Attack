...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        self.negative = self.params.NEGATIVE
        self.negative_policy = []
        self.negative_policies = []
        self.kls = []
    ...
    def multi_actor_step(self, actions, envs, save_frames):
        '''
        ...
        for action, env in zip(actions, envs):
            ...
            # print(self.negative)
            if self.negative:
                # print("old: ", normed_reward)
                normed_reward = -normed_reward
                # print("new: ", normed_reward)

            if is_done:
                # if self.negative:
                #     info_done = (info['done'][0], -info['done'][1])
                # else:
                #     info_done = info['done']
                info_done = info['done']
                completed_episode_info.append(info_done)
                # completed_episode_info.append(info['done'])
                # print("complete", completed_episode_info)
                new_state = env.reset()
        ...
        return [completed_episode_info, *data], frames
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        for t in iterator:
            ...
            if len(done_info) > 0 and (t != self.T - 1 or len(completed_episode_info) == 0):
                ...
                # print("info", completed_episode_info)
    ...
    def apply_attack(self, last_states):
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            if steps > 0:
                ...
                return last_states
        elif self.params.ATTACK_METHOD == "target_action":
            if steps > 0:
                if self.params.ATTACK_STEP_EPS == "auto":
                    step_eps = eps / steps
                else:
                ...
                # print(old_action)
                target_action = torch.zeros(old_action.shape)
                # print(target_action)
                ...
                with torch.enable_grad():
                    for i in range(steps):
                        ...
                        action_change = (self.policy_model(states)[0] - target_action) / old_stdev
                ...
                return last_states
        elif self.params.ATTACK_METHOD == "target_action_advpolicy":
            if steps > 0:
                if self.params.ATTACK_STEP_EPS == "auto":
                    step_eps = eps / steps
                else:
                ...
                # Clamp using tanh.
                # hardtanh: >1 to 1, < -1 to -1, else stay the same
                perturbed_states = last_states + ch.nn.functional.hardtanh(perturbations_mean) * eps
                target_action, stdev = self.policy_model(perturbed_states)
                ...
                with torch.enable_grad():
                    for i in range(steps):
                        states = states.clone().detach().requires_grad_()
                        # action_change = (self.policy_model(states)[0] - old_action) / old_stdev
                        action_change = (self.policy_model(states)[0] - target_action) / stdev
                        action_change = (action_change * action_change).sum(dim=1)
                        action_change.backward()
                        # Reduce noise at every step.
                        noise_factor = np.sqrt(2 * step_eps) / (i + 2)
                ...
                attacked_action, attacked_std = self.policy_model(states)
                self.policy_model.zero_grad()
                # print("target action is: ", target_action)
                # print("attacked action is: ", attacked_action)
                return states.detach()
            else:
                return last_states
        elif self.params.ATTACK_METHOD == "target_action_negpolicy":
            if steps > 0:
                if self.params.ATTACK_STEP_EPS == "auto":
                    step_eps = eps / steps
                else:
                ...
                # Attack using a negative policy.
                # negative_policy = policy_net_with_name(self.params['policy_net_type'])
                # pretrained_model = torch.load(self.params['load_model'])
                # negative_policy.load_state_dict(pretrained_model['policy_model'])

                # negative_policy = self.policy_model
                # Unlike other attacks we don't need step or eps here.
                # We don't sample and use deterministic adversary policy here.

                # target_action, stdev = self.negative_policy[0](last_states)
                # for i in range(len(self.negative_policy)-1):
                #     now_target_action, now_stdev = self.negative_policy[i+1](last_states)
                #     target_action = target_action + now_target_action
                #     stdev = stdev + now_stdev
                # target_action = target_action / len(self.negative_policy)
                # stdev = stdev / len(self.negative_policy)

                target_action, stdev = self.negative_policy(last_states)
                # print("target action is: ", target_action)
                stdev /= (stdev.mean())
                ...
                with torch.enable_grad():
                    for i in range(steps):
                        ...
                        # Project noisy gradient to step boundary.
                        update = (states.grad + noise_factor * torch.randn_like(last_states)).sign() * step_eps
                        # update = states.grad + noise_factor * torch.randn_like(last_states)
                        # update = update / update.abs().max()
                        # update = update * step_eps
                        # Clamp to +/- eps.
                        states.data = torch.min(torch.max(states.data - update, clamp_min), clamp_max)
                    self.policy_model.zero_grad()

                # update = states - last_states
                # if update.abs().max() > 0.0:
                #     update = update / update.abs().max()
                # states = last_states + update * eps

                attacked_action, attacked_std = self.policy_model(states)
                action_diff = attacked_action - target_action
                now_kl = (action_diff * action_diff).sum().item()
                # now_kl = (action_diff * action_diff).sum().item() \
                #          + stdev.sum().item() \
                #          - torch.log(torch.prod(stdev)).item()
                self.kls.append(now_kl)
                self.policy_model.zero_grad()
                # print("last states is: ", last_states)
                # print("disturbed states is: ", states)
                # print("target action is: ", target_action)
                # print("attacked action is: ", attacked_action)
                return states.detach()
            else:
                return last_states
        elif self.params.ATTACK_METHOD == "target_action_negpolicies":
            if steps > 0:
                # eps = 0.1
                # self.params.ATTACK_EPS = 0.1
                if self.params.ATTACK_STEP_EPS == "auto":
                    step_eps = eps / steps
                else:
                    step_eps = float(self.params.ATTACK_STEP_EPS)

                # Attack using many negative policies.
                # negative_policy = policy_net_with_name(self.params['policy_net_type'])
                # pretrained_model = torch.load(self.params['load_model'])
                # negative_policy.load_state_dict(pretrained_model['policy_model'])
                ...
                # print('lalala')
                kls = []
                for j in range(len(self.negative_policies)):
                    target_action, stdev = self.negative_policies[j](last_states)
                    # print("target action is: ", target_action)
                    stdev /= (stdev.mean())
                    ...
                    attacked_action, attacked_std = self.policy_model(states)
                    attacked_std /= (attacked_std.mean())
                    self.policy_model.zero_grad()
                    # print("last states is: ", last_states)
                    # print("disturbed states is: ", states)
                    # print(j)
                    # print("states: ", states)
                    # print("target action is: ", target_action)
                    # print("traget std is: ", stdev)
                    # print("attacked action is: ", attacked_action)
                    # print("attacked std is: ", attacked_std)
                    action_diff = attacked_action - target_action
                    now_kl = (action_diff * action_diff).sum().item() \
                             + stdev.sum().item() \
                             - torch.log(torch.prod(stdev)).item()
                    # now_kl = now_kl.item()
                    kls.append(now_kl)
                    output_stateses.append(states)

                    # now_dis = torch.norm(attacked_action - target_action).item()
                    # print(now_dis)
                    # if now_dis < dis:
                    #     dis = now_dis
                    #     output_states = states
                # print("index: ", index)
                output_states = last_states
                kl = 0.0
                if eps == 0.0:
                ...
                for i in range(len(kls)):
                    kl += kls[i]
                for i in range(len(kls)):
                    # print(output_stateses[i])
                    output_states = output_states + \
                                    (output_stateses[i] - last_states) * kls[i] / kl

                output_states = last_states + \
                                eps * (output_states - last_states) / (output_states - last_states).abs().max()
                # print("output states", output_states)
                # print("last states", last_states)
                return output_states.detach()
            else:
                return last_states
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None):
        ...
        with torch.no_grad():
            ...
            ep_length, ep_reward, actions, action_means, states, frames = output
            ...
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, actions.cpu().numpy(), action_means.cpu().numpy(), states.cpu().numpy(), kl_upper_bound, frames
    ...
    def run_test_trajectories(self, max_len, should_tqdm=False):
        ...
        for t in iterator:
            ...
            ret = self.multi_actor_step(next_actions, envs, save_frames=need_save_frames)

            # done_info = List of (length, reward) pairs for each completed trajectory
            # (next_rewards, next_states, next_dones) act like multi-actor env.step()
            (done_info, next_rewards, next_states, next_not_dones), __frames = ret

            if need_save_frames:
                frames.append(__frames[0])
        ...
        to_ret = (ep_length, ep_reward, actions, action_means, states, frames)

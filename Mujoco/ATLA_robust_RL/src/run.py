...
from policy_gradients.torch_utils import cu_tensorize, cpu_tensorize
...
def main(params):
    ...
    ## old_p: the same as p
    old_p = Trainer.agent_from_params(params, store=store)
    if params['initial_std'] != 1.0:
        old_p.policy_model.log_stdev.data[:] = np.log(params['initial_std'])
    if 'load_model' in params and params['load_model']:
        print('Loading pretrained model', params['load_model'])
        pretrained_model = torch.load(params['load_model'])
        if 'policy_model' in pretrained_model:
            old_p.policy_model.load_state_dict(pretrained_model['policy_model'])
        if params['deterministic']:
            print('Policy runs in deterministic mode. Ignoring Gaussian noise.')
            old_p.policy_model.log_stdev.data[:] = -100
        else:
            print('Policy runs in non deterministic mode with Gaussian noise.')
        if 'val_model' in pretrained_model:
            old_p.val_model.load_state_dict(pretrained_model['val_model'])
        if 'policy_opt' in pretrained_model:
            old_p.POLICY_ADAM.load_state_dict(pretrained_model['policy_opt'])
        if 'val_opt' in pretrained_model:
            old_p.val_opt.load_state_dict(pretrained_model['val_opt'])
        # Load adversary models.
        if 'no_load_adv_policy' in params and params['no_load_adv_policy']:
            print('Skipping loading adversary models.')
        else:
            if 'adversary_policy_model' in pretrained_model and hasattr(p, 'adversary_policy_model'):
                old_p.adversary_policy_model.load_state_dict(pretrained_model['adversary_policy_model'])
            if 'adversary_val_model' in pretrained_model and hasattr(p, 'adversary_val_model'):
                old_p.adversary_val_model.load_state_dict(pretrained_model['adversary_val_model'])
            if 'adversary_policy_opt' in pretrained_model and hasattr(p, 'adversary_policy_opt'):
                old_p.adversary_policy_opt.load_state_dict(pretrained_model['adversary_policy_opt'])
            if 'adversary_val_opt' in pretrained_model and hasattr(p, 'adversary_val_opt'):
                old_p.adversary_val_opt.load_state_dict(pretrained_model['adversary_val_opt'])
        if 'envs' in pretrained_model:
            old_p.envs = pretrained_model['envs']
        for e in old_p.envs:
            e.setup_visualization(params['show_env'], params['save_frames'], params['save_frames_path'])
    ## old_p: the same as p
    ...
    try:
        ...
        f = open(store.path + '/result.txt', "w")
        print("steps", file=f)
        ...
        for i in range(params['train_steps']):
            ...
            if params['negative']:
                if i % 10 == 9:
                    ...
                    now_attacked_target_kls = []
                    now_attacked_old_kls = []
                    now_target_old_kls = []
                    print("This is steps ", i, ". Now we will calculate the kl.")
                    for j in range(5):
                        iterator = range(1000)
                        envs = p.envs
                        last_states = p.reset_envs(envs)
                        p.policy_model.reset()
                        p.val_model.reset()
                        for t in iterator:
                            if t > 0 and t % 100 == 0:
                                print("This is trajectory", j, "step ", t)

                            with torch.no_grad():
                                action_pds = p.policy_model(last_states)
                                # action_pds = action, std

                            ###   calculate kls  ###
                                # target_action = action_pds[0]
                                old_p_action = old_p.policy_model(last_states)[0]
                                # print("p", target_action)
                                # print("old_p", old_p_action)

                                eps = float(0.15)
                                steps = p.params.ATTACK_STEPS
                                if p.params.ATTACK_STEP_EPS == "auto":
                                    step_eps = eps / steps
                                else:
                                    step_eps = float(p.params.ATTACK_STEP_EPS)
                                target_action, stdev = action_pds
                                # print("target action is: ", target_action)
                                stdev /= (stdev.mean())
                            ...
                            with torch.enable_grad():
                                for k in range(steps):
                                    ...
                                    action_change = (old_p.policy_model(states)[0] - target_action) / stdev
                                    action_change = (action_change * action_change).sum()
                                    # print(action_change)
                                    action_change.backward()
                                    # Reduce noise at every step.
                                    noise_factor = np.sqrt(2 * step_eps) / (k + 2)
                                    # Project noisy gradient to step boundary.
                                    update = (states.grad + noise_factor * torch.randn_like(
                                        last_states)).sign() * step_eps
                                    states.data = torch.min(torch.max(states.data - update, clamp_min), clamp_max)
                                old_p.policy_model.zero_grad()

                            attacked_action, attacked_std = old_p.policy_model(states)

                            action_diff1 = attacked_action - target_action
                            attacked_target_kl = (action_diff1 * action_diff1).sum().item()
                            action_diff2 = attacked_action - old_p_action
                            attacked_old_kl = (action_diff2 * action_diff2).sum().item()
                            action_diff3 = target_action - old_p_action
                            target_old_kl = (action_diff3 * action_diff3).sum().item()
                            old_p.policy_model.zero_grad()
                            now_attacked_target_kls.append(attacked_target_kl)
                            now_attacked_old_kls.append(attacked_old_kl)
                            now_target_old_kls.append(target_old_kl)
                            # print("old p action", old_p_action)
                            # print("target action", target_action)
                            # print("attacked action", attacked_action)
                            #
                            # print("kl between attacked and target", attacked_target_kl)
                            # print("kl between attacked and old", attacked_old_kl)
                            # print("kl between target and old", target_old_kl)
                            ...
                            tensor_maker = cpu_tensorize if p.CPU else cu_tensorize
                            next_rewards, next_states = map(tensor_maker, [normed_rewards, states])
                            last_states = next_states[:, 0, :]
                            # print("rewards", next_rewards)
                            # print("actions", next_actions)
                            if is_done:
                                break

                    now_attacked_target_kls = np.array(now_attacked_target_kls)
                    now_attacked_old_kls = np.array(now_attacked_old_kls)
                    now_target_old_kls = np.array(now_target_old_kls)

                    mean_attacked_target_kl = np.mean(now_attacked_target_kls)
                    mean_attacked_old_kl = np.mean(now_attacked_old_kls)
                    mean_target_old_kl = np.mean(now_target_old_kls)

                    attacked_target_kls.append(mean_attacked_target_kl)
                    attacked_old_kls.append(mean_attacked_old_kl)
                    target_old_kls.append(mean_target_old_kl)

                    print("kl between attacked and target", mean_attacked_target_kl)
                    print("kl between attacked and old", mean_attacked_old_kl)
                    print("kl between target and old", mean_target_old_kl)
                    print("This is steps ", i, ". We have calculated the kl.")
        ...
        print("all attacked target kls", attacked_target_kls)
        print("all attacked old kls", attacked_old_kls)
        print("all target old kls", target_old_kls)

        print('', file=f)
        print("all attacked target kls", file=f)
        for i in attacked_target_kls:
            print(i, end=" ", file=f)
        print('', file=f)
        print("all attacked old kls", file=f)
        for i in attacked_old_kls:
            print(i, end=" ", file=f)
        print('', file=f)
        print("all target old kls", file=f)
        for i in target_old_kls:
            print(i, end=" ", file=f)
        print('', file=f)
...
def add_common_parser_opts(parser):
    ...
    parser.add_argument('--negative_id', type=str, default='')
    parser.add_argument('--attack_methods', type=str, default='')
    parser.add_argument('--task_algorithm', type=str, default='ant_vanilla_ppo')
    ...
    parser.add_argument('--attack-method', type=str, choices=["none", "critic", "random", "action", "sarsa",
                                                              "sarsa+action", "advpolicy", "action+imit",
                                                              "target_action", "target_action_advpolicy",
...
if __name__ == '__main__':
    ...
    extra_params = ['config_path', 'out_dir_prefix', 'load_model', 'no_load_adv_policy',
                    'adv_policy_only', 'deterministic', 'seed', 'negative', 'negative_id',
                    'attack_methods', 'task_algorithm']

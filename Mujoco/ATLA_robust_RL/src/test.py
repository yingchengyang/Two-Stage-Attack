...
def main(params):
    ...
    if params['negative']:
        ...
        for negative_id in negative_ids:
            ...
            p.negative_policy=neg_p.policy_model
            store.close()
    ...
    if params['sarsa_enable']:
        ...
        for i in range(num_episodes):
            ...
            ep_length, ep_reward, actions, action_means, states, kl_certificates, frames = p.run_test(compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=original_stdev)
            if i == 0:
                all_actions = actions.copy()
                all_states = states.copy()
            else:
        ...
        print(len(frames))
        # import imageio
        # with imageio.get_writer(os.path.join(save_path, 'video.mp4'), fps=30) as video:
        #     for frame in frames:
        #     for frame in frames:
